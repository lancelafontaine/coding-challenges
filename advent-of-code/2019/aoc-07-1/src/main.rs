...
impl AmplifierSystem {
    ...
    fn compute(&mut self, phases: Vec<u8>, input: i64) -> i64 {
        ...
        for amplifier in &mut self.amplifiers {
            amplifier.compute().unwrap();
        }
        self.global_receiver.recv().unwrap()
    }
}
...
fn main() -> Result<()> {
    ...
    let mut phases = (0..=4).collect::<Vec<u8>>();
    ...
    let output = phase_permutations
        ...
        .map(|permutation| {
            ...
            amplifier_system.compute(permutation, 0)
        })
        .max()
        .unwrap();

...
impl Point {
    ...
    fn _manhattan_distance(&self, other: &Point) -> u64 {
...
impl Line {
    ...
    fn distance_until(&self, point: &Point) -> (bool, u64) {
        let (hit_intersection, end_point) = if let Some(intersection) =
            self.get_intersection(&Line::new(Orientation::Unidimensional, *point, *point))
        {
            (true, intersection)
        } else {
            (false, self.end)
        ...
        let distance = if self.orientation == Orientation::Horizontal {
            self.start.x - end_point.x
        } else {
            self.start.y - end_point.y
        }
        .abs()
        .try_into()
        .unwrap();
...
fn main() -> Result<()> {
    ...
    let output = intersections
        ...
        .map(|intersection| {
            let mut sum = 0;
            for wire_lines in &lines_of_wires {
                for wire_line in wire_lines {
                    let (hit_intersection, distance) = wire_line.distance_until(&intersection);
                    sum += distance;
                    if hit_intersection {
                        break;
                    }
...
fn get_lines_of_wire(steps_of_wires: Vec<Vec<Step>>) -> Vec<Vec<Line>> {
    ...
    for wire_number in 0..=1 {
        ...
        for &step in wire_steps {
            let line: Line = StepLine::new(point, step).into();
            point.x = line.end.x;
            point.y = line.end.y;
